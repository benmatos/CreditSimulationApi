...
namespace CreditsimulacaoApi.Controllers
{
    ...
    public class simulacaoController : ControllerBase
    {
        ...
        private readonly ISimulacaoPersistenceService _simulacaoPersistenceService;
        ...
        public simulacaoController(ProductDbContext productContext, SimulationDbContext simulationContext, ISimulacaoPersistenceService simulacaoPersistenceService, IEventHubJsonWriter eventHubWriter)
        {
            _productContext = productContext;
            _simulationContext = simulationContext;
            _eventHubWriter = eventHubWriter;
            _simulacaoPersistenceService = simulacaoPersistenceService;
        }
        ...
        [HttpPost("simulate")]
        public async Task<ActionResult<simulacaoResponse>> Simulation([FromBody] simulacaoRequest request)
        {
            try
            {
                ...
                for (int i = 1; i <= prazo; i++)
                {
                    ...
                    valorTotalParcelas += Math.Round(priceParcelas, 2);
                }
                ...
                for (int i = 1; i <= prazo; i++)
                {
                    ...
                    valorTotalParcelas += Math.Round(Parcelas, 2);
                }
                ...
                //Persist simulação
                var simulacao = await _simulacaoPersistenceService.PersistirSimulacaoAsync(product, valorDesejado, prazo, tempoExecucao, sucesso, valorTotalParcelas);
        ...
        public async Task<ActionResult<object>> GetSimulatedVolumes()
        {
            var dataReferencia = DateTime.UtcNow.ToString("yyyy-MM-dd");

            var simulacoes = await _simulationContext.Simulacoes
                .GroupBy(s => new { s.idProduto, s.noProduto })
                .Select(g => new
                {
                    codigoProduto = g.Key.idProduto,
                    descricaoProduto = g.Key.noProduto,
                    taxaMediaJuro = Math.Round(g.Average(s => s.TaxaJuros), 3),
                    valorMedioPrestacao = Math.Round(g.Average(s => (s.valorDesejado + (s.TaxaJuros * s.valorDesejado * s.prazo)) / s.prazo), 2),
                    valorTotalDesejado = Math.Round(g.Sum(s => s.valorDesejado), 2),
                    valorTotalCredito = Math.Round(g.Sum(s => s.valorDesejado + (s.TaxaJuros * s.valorDesejado * s.prazo)), 2)
                })
                .ToListAsync();
